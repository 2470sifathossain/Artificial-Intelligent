...
while time.time() < future:
    ...
    if len(dets) > 0:
        for k, d in enumerate(dets):
            ...
            cv2.putText(frame, "Total Count: {}".format(total), (410, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 0, 255),
                        2)
            cv2.putText(frame, "Time: {}".format(int(future - time.time())), (10, 410), cv2.FONT_HERSHEY_SIMPLEX, 0.7,
                        (255, 0, 255), 2)
            for (x, y) in shape:
